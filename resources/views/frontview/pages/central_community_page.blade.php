                ...
                <div class="col-lg-3 my-2">
                    <form action="" method="" id="searchComitee" class="d-flex">
                        <select class="form-select" name="duration" id="duration" aria-label="Default select example" required>
                            ...
                            @foreach ( App\Models\CentralComitee::distinct()->get('duration') as $centralComitee )
                                <option value="{{ $centralComitee->duration }}">{{ $centralComitee->duration }}</option>
                            @endforeach
                        </select>
                ...
                @foreach ($centralComitees as $centralComitee)
                    <div class="col-lg-3 col-sm-6 my-3">
                        <div class="card h-100">
                            ...
                            <div class="card-body ">
                                <h5 class="card-text text-center"><strong>{{ $centralComitee->comitee_designation }}</strong></h5>
                                <h6 class="card-text text-start"><strong>Name:</strong> {{ $centralComitee->name }}</h6>
                                <h6 class="card-text text-start"><strong>Designation:</strong> {{ $centralComitee->designation }}</h6>
                                <h6 class="card-text text-start"><strong>Bank Name:</strong> {{ $centralComitee->bank_name }}</h6>
                                @if ($centralComitee->bankUser)
                                    <h6 class="card-text text-start"><strong>Email:</strong> {{ $centralComitee->bankUser->email }}</h6>
                                @endif
                                <h6 class="card-text text-start" style="font-family:Arial;"><strong>Phone:</strong> {{ $centralComitee->mobile_no }}</h6>
                            </div>
                        </div>
                    </div>
                @endforeach
