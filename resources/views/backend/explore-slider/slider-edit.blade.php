...
@section('title', 'Edit Explore Product Slider')
      			...
      			<div class="col-lg-12">
          			<div class="card card-primary card-outline ">
			            <div class="card-header">
			                <h5 class="m-0">Edit Explore Product Slider</h5>
			            </div>

		              	<div class="card-body">
	          				<div class="container">

								<form method="POST" action="{{route('explore.slider.edit',$data->id )}}" enctype="multipart/form-data">
                                      @csrf
                                      <input type="hidden" class="form-control" id="category_name" name="category_name" value="{{$data->category_name}}">
                                      ...
                                      <select class="form-control w-50 selectpicker " name="category_id"
                                          ...
                                          <option value="{{ $category->id }}"{{$category->id==$data->category_id ? 'selected' : ''}}>{{ $category->category_name }}
                                          </option>
                                          @endforeach
                                      </select>
                                      ...
                                      <div class="mb-3">
                                        <div class="form-group">
                                            ...
                                            <textarea class="form-control"  id="slider_url" name="slider_url" rows="3" placeholder="Enter Slider Description" > {{$data->description}}</textarea>
                                          </div>
                                      </div>
