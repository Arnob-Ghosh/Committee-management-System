$('.item-hover-variant-image a').hover(
    ...
    function () {
        ...
        var first_image = image.replace('' + base_url + '/uploads/product/smartphone/variant/', '');


        $.ajax({
            type: "GET",
            url: "/get-smart-phone-variant-images/" + first_image,
...
$(document).on('click', '.checkbox', function () {
    ...
    if (displayList.length == 0 && cameraList.length == 0 && batteryList.length == 0 && networkList.length == 0) {
        $('#product-loaded').show();
...
function submitToServer(cameraList, displayList, batteryList, networkList) {
    ...
    $.ajax({
        ...
        url: "/smart-phone/get-device-by-display/" + cameraList + '/' + displayList + '/' + batteryList + '/' + networkList,
            ...
            if (response.data == '') {
                ...
                <div class="product product-7 text-center">\
                    <figure class="product-media">\
                        <a href="/smart-phone-view/'+ item.id + '" id="display-append-img-div-' + i + '" class="display-append-item-hover">\
                            <img src="' + item.smart_phone[0].back_image +
                        '" alt="Product image" class="record" id="display-append-record' + i + '" style="position: absolute;left: 50px;bottom: 0px;z-index: 1;">\
                            <img src="' + item.smart_phone[0].front_image +
                        '" alt="Product image" class="sleeve" id="display-append-sleeve' + i + '" style=" position: relative;left: 80px;bottom: 0px;z-index: 1;">\
                        </a>\
                    </figure>\
                    <div class="product-body">\
                        <h2 class="product-title"><a href="/smart-phone-view/'+ item.id + '">' + item.model_name +
                        '</a></h2>\
...
$(document).on('click', '.brand-sort', function () {
    ...
    $.ajax({
        type: "get",
        url: "/smart-phone/get-device-by-brand/" + brand_id,
        contentType: false,
        processData: false,
        success: function (response) {
            $('#product-loaded').hide();
        ...
        ');
            ...
            <div class="product product-7 text-center">\
                <figure class="product-media">\
                    <a href="/smart-phone-view/'+ item.id + '" id="brand-append-img-div-' + i + '" class="brand-append-item-hover">\
                        <img src="' + item.smart_phone[0].back_image +
                        '" alt="Product image" class="record" id="brand-append-record' + i + '" style="position: absolute;left: 50px;bottom: 0px;z-index: 1;">\
                        <img src="' + item.smart_phone[0].front_image +
                        '" alt="Product image" class="sleeve" id="brand-append-sleeve' + i + '" style=" position: relative;left: 80px;bottom: 0px;z-index: 1;">\
                    </a>\
                </figure>\
                <div class="product-body">\
                    <h2 class="product-title"><a href="/smart-phone-view/'+ item.id + '">' + item.model_name +
                        '</a></h2>\
                    <div class="product-nav product-nav-thumbs brand-append-item-hover-variant-image" id="brand-append-variant-image-' +
                        i + '">\
                    </div>\
                </div>\
            </div>\
 ...
 ');
                    $.each(item.smart_phone, function (index, data) {
                        ...
                        $('#brand-append-variant-image-' + i + '').append('<a href="javascript:void(0)" id="brand-append-ab-' + i + '" class="append-active">\
    <img src="' + data.colour_thumbnail + '" alt="product desc" style="width: 40px;height:40px;">\
</a>\
    ');
                    });
