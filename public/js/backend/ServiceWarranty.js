$(document).on('click', '.create-btn', function (e) {
    e.preventDefault();
    $('#CreateServiceWarrantyModal').modal('show');

});

$(document).on('click', '#close', function (e) {
    $('#CreateServiceWarrantyModal').modal('hide');
...
$(document).ready(function () {
    ...
    var count = 1;
    //CREATE ServiceWarranty
    $(document).on('submit', '#CreateServiceWarrantyForm', function (e) {
        e.preventDefault();

        let formData = new FormData($('#CreateServiceWarrantyForm')[0]);

        $.ajax({
            type: "POST",
            url: "/service-warranty-create",
            data: formData,
            contentType: false,
            processData: false,
            success: function (response) {
                ...
                if ($.isEmptyObject(response.error)) {
                    $('#CreateServiceWarrantyModal').modal('hide');
                    $('#CreateServiceWarrantyModal form :input').val("");
                   ...
                   tinymce.get(tinymce_editor_id).setContent('');
                    ...
                    count = 1;
                    $('#service_warranty_table').DataTable().ajax.reload();
    ...
    $("#service_warranty_table").DataTable({
        responsive: true,
        ajax: {
            url: "/service-warranty-list-data",
            dataSrc: "data",
        },
    ...
    $("#service_warranty_table").on("click", ".edit_btn", function (e) {
        e.preventDefault();

        var id = $(this).val();
        // alert(id)
        $('#EditServiceWarrantyModal').modal('show');

        $.ajax({
            type: "GET",
            url: "/service-warranty-edit/" + id,
            success: function (response) {
    ...
    $(document).on('submit', '#EditServiceWarrantyForm', function (e) {
        e.preventDefault();

        var id = $('#get_ServiceWarranty_id').val();

        let EditFormData = new FormData($('#EditServiceWarrantyForm')[0]);
    ...
    $("#service_warranty_table").on("click", ".dlt_btn", function (e) {
        e.preventDefault();

        var id = $(this).val();
        $('#DeleteServiceWarrantyModal').modal('show');
        $('#ServiceWarranty_id').val(id);
        $(document).on("click", ".confrim_dlt", function () {
            $.ajax({
                type: "get",
                url: "/service-warranty-delete/" + id,
                contentType: false,
                processData: false,
                success: function (response) {
                    // console.log(response.message);
                    $('#DeleteServiceWarrantyModal').modal('hide');
                    $.notify(response.message, 'success')
                    count = 1;
                    $('#service_warranty_table').DataTable().ajax.reload();
                }
