$('select[name="brand_id"]').on('change', function () {
    ...
    $.ajax({
        url: "/get-feature-phone-model/ajax/" + brand_id,
        type: "GET",
        dataType: "json",
...
function overviewImageAddToServer() {
    ...
    if ($('#specification_transfer_table tr').length > 1) {
        ...
        $(overviewImageTable).find('> tbody > tr').each(function () {
            ...
            // overviewImage["upper_image_file"] =  formData.append("upper_image_file", $(this).find("td:eq(2) input[type='file']").get(0).files[0]);
            // overviewImage["lower_image_file"] =  formData.append("lower_image_file", $(this).find("td:eq(3) input[type='file']").get(0).files[0]);





            // formData.append("upper_image_file", $(this).find("td:eq(2) input[type='file']").get(0).files[0]);
            // formData.append("lower_image_file", $(this).find("td:eq(3) input[type='file']").get(0).files[0]);
...
function productTransfer(jsonData) {
    ...
    $.ajax({
        type: "POST",
        url: "/feature-phone/over-view-image-create",
        ...
        success: function (response) {
            ...
            $(location).attr("href", "/feature-phone/over-view-image-list");
...
function overviewImageImageStore() {
    ...
    //     $.ajax({
    //         type: 'POST',
    //         url: "/feature-phone/overview-image-store",
    //         data: formData,
    //         cache: false,
    //         contentType: false,
    //         processData: false,
    ...
    $(T).find('> tbody > tr').each(function (){
        var a =$(this).find("td:eq(2) input[type='file']").val().split('\\').pop()
...
$(document).on("click", ".edit_btn", function (e) {
    ...
    window.location.href = "/feature-phone/overviewImage-edit/" + modelid;
});
...
$(document).ready(function () {
    $('#overviewImage_table').on('click', '.delete_btn', function () {
        ...
        $('#DELETEFoverviewImageFORM').attr('action', '/feature-phone/overviewImage-delete/' + overviewImageid);
